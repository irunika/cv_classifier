...
def generate_candidate_keywords(sentence_list, stopword_pattern, min_char_length=1, max_words_length=10):
    phrase_list = []
    for s in sentence_list:
        ...
        tmp = ''
        for word in tempS:
            if(word!='IS' and word!='AS'):
                word = re.sub(stopword_pattern, '|', word.strip())
            tmp += word + ' '
        tmp.strip()
        phrases = tmp.split("|")
...
def calculate_word_scores(phraseList):
    ...
    for phrase in phraseList:
        ...
        for word in word_list:
            ...
            if smart_str(word).lower() in wso2_words:
                word_degree[word] += 2 + word_list_degree
            else:
